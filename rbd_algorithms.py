...
class RBDAlgorithms:
    ...
    def crba(
        ...
        for i, node in reversed(list(enumerate(self.model.tree))):
            ...
            while link_j.name != self.root_link: # originario: link_j.name != ma penso sia sbagliato xchè fatto prova e rimane link_j=link_i mentre da algoritmo dovrei avere il padre di j.
                F = X_p[j].T @ F
    ...
    def rnea(
        ...
        tau = self.math.factory.zeros(self.NDoF + 6, 1) # NDoF=len(joints_name_list)
        model_len = self.model.N #set the "length of the model as the number of links(corpi)"
        ...
        for i, node in enumerate(self.model.tree):
            ...
            if link_i.name == self.root_link:
                ...
                X_p[i] = self.math.spatial_transform(
                    self.math.factory.eye(3), self.math.factory.zeros(3, 1)
                ) #la matrice di trasformazione spaziale per la base non cambia nulla
                Phi[i] = self.math.factory.eye(6)
                ...
                Phi[i] = joint_i.motion_subspace() #per joint a terra (sostituito con 6-dof) ha una matrice 6*6 per altri joint un vettore 6*1 con un solo valore =1
                pi = self.model.tree.get_idx_from_name(link_pi.name)
            ...
            f[i] = Ic[i] @ a[i] + self.math.spatial_skew_star(v[i]) @ Ic[i] @ v[i] # f grande come numero corpi e ogni posizione un vettore di stesso numero di joint

        for i, node in reversed(list(enumerate(self.model.tree))):
            node: Node
            link_i, joint_i, link_pi = node.get_elements()
            if link_i.name == self.root_link:
                tau[:6] = Phi[i].T @ f[i] # moltiplicazine da vettore 6*6 perchè in caso di root_link phi[i] è matrice 6*6
            elif joint_i.idx is not None:
